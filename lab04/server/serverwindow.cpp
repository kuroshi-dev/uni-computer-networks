...
ServerWindow::ServerWindow(QWidget *parent): QMainWindow(parent), ui(new Ui::ServerWindow), tcpServer(nullptr)
{
    ui->setupUi(this);

    ui->portSpinBox->setValue(3074);
    ...
    logMessage("Server is ready for start");
}
...
void ServerWindow::onStartServer()
{
    ...
    if (tcpServer->listen(QHostAddress::Any, port)) {
        ...
        updateServerInfo();
        monitorTimer->start(3000); // client check
        ...
        QMessageBox::critical(this, "Error",
                              "Failed to launch server: " + tcpServer->errorString());
    }
}
...
{
    QString info = QString("IP adress: %1\nPort: %2\nStatus: Searching for connections...")
                       .arg(getLocalIP())
...
void ServerWindow::onNewConnection()
{
    ...
    logMessage(QString("> New client connected: IP: %1, Port: %2, Status: Online")
                   .arg(info.ipAddress).arg(info.port));
...
void ServerWindow::onReadyRead()
{
    ...
    if (message == "HEARTBEAT") {
        ...
        // Informational packet data
        parseDataPacket(message.mid(5), clientSocket);
    }
}
...
void ServerWindow::parseDataPacket(const QString &data, QTcpSocket *client)
{
    ...
    if (fields.size() != 5) {
        logMessage("❌ Error: Wrong packet data format");
        return;
    }
    ...
    case 1: specName = "Computer Networks"; break;
    case 2: specName = "Databases"; break;
    case 3: specName = "Cybersecurity"; break;
    case 4: specName = "Cloud"; break;
    default: specName = "???";
    }
    ...
    logMessage("\n" + QString("=").repeated(60));
    logMessage("📦 INFORMATION PACKAGE RECEIVED");
    logMessage(QString("From client: %1:%2").arg(info.ipAddress).arg(info.port));
    logMessage(QString("─").repeated(60));
    logMessage(QString("Line 1 (Surname):      %1").arg(surname));
    logMessage(QString("Line 2 (Phone number):       %1").arg(phone));
    logMessage(QString("Line 3 (Age):           %1").arg(age));
    logMessage(QString("Line 4 (Specialization): %1 (%2)").arg(specialization).arg(specName));
    logMessage(QString("Line 5 (Description):          %1").arg(description));
    logMessage(QString("=").repeated(60) + "\n");
}
...
{
    if (clients.isEmpty()) {
        ui->clientsTextEdit->setPlainText("Haven't any connections");
        return;
    }

    QString list = QString("=== List of current connections (%1) ===\n\n").arg(clients.size());
    ...
    foreach (auto client, clients.keys()) {
        ...
        bool isActive = secondsSinceLastSeen < 15; //if not accessible more that 15 sec, then offline

        list += QString("Client #%1:\n").arg(index++);
        list += QString("  IP adress: %1\n").arg(info.ipAddress);
        list += QString("  Port: %1\n").arg(info.port);
        list += QString("  Status: %1\n").arg(isActive ? "🟢 Online" : "🔴 Offline");
        list += QString("  Last connection: %1 sec. ago\n\n")
